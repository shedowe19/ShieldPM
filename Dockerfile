...
RUN apt-get update && apt-get install -y --no-install-recommends nodejs npm binutils file curl && \
    ...
    curl -L "https://github.com/oauth2-proxy/oauth2-proxy/releases/download/v7.14.2/sha256sum.txt" -o /tmp/oauth2-proxy-sha256sum.txt && \
    cd /tmp && grep "oauth2-proxy-v7.14.2.linux-${TARGETARCH}.tar.gz" oauth2-proxy-sha256sum.txt | sha256sum -c && cd - && \
    ...
    rm "/tmp/oauth2-proxy-v7.14.2.linux-${TARGETARCH}.tar.gz" /tmp/oauth2-proxy-sha256sum.txt && \

...
RUN apt-get update && apt-get install -y --no-install-recommends nodejs npm binutils file curl && \
    ...
    cd /tmp && \
    tar -xzf "oauth2-proxy-v7.14.2.linux-${TARGETARCH}.tar.gz" && \
    sha256sum -c "oauth2-proxy-v7.14.2.linux-${TARGETARCH}-sha256sum.txt" && \
    mv "oauth2-proxy-v7.14.2.linux-${TARGETARCH}/oauth2-proxy" /app/oauth2-proxy && \
    rm -rf "oauth2-proxy-v7.14.2.linux-${TARGETARCH}" "oauth2-proxy-v7.14.2.linux-${TARGETARCH}.tar.gz" "oauth2-proxy-v7.14.2.linux-${TARGETARCH}-sha256sum.txt" && cd - && \

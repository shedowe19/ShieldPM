...
RUN apt-get update && apt-get install -y --no-install-recommends nodejs npm binutils file curl && \
    ...
    cd /tmp && sha256sum -c "oauth2-proxy-v7.14.2.linux-${TARGETARCH}-sha256sum.txt" && cd - && \
    ...
    rm "/tmp/oauth2-proxy-v7.14.2.linux-${TARGETARCH}.tar.gz" "/tmp/oauth2-proxy-v7.14.2.linux-${TARGETARCH}-sha256sum.txt" && \

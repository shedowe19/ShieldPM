...
export function useMyFeatures() {
    return useQuery({ queryKey: ["my-features"], queryFn: () => api.get("/api/my-features") });
}
...
- **Nginx validation disabled**: `nginx -t` is skipped before reload for speed. Template errors can break Nginx.
...
- Nginx reload is debounced (2-second delay). Rapid config changes will batch into a single reload.

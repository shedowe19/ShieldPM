...
if [[ "$oauth2_choice" =~ ^[Yy]$ ]]; then
    ...
    if [ "$SHOULD_UPDATE_OAUTH2" = true ]; then
        ...
        OAUTH2_SHA_URL="https://github.com/oauth2-proxy/oauth2-proxy/releases/download/v${OAUTH2_VERSION}/oauth2-proxy-v${OAUTH2_VERSION}.linux-${OAUTH2_ARCH}-sha256sum.txt"
        
        OAUTH2_TMP_DIR=$(mktemp -d)
        OAUTH2_TAR="$OAUTH2_TMP_DIR/$OAUTH2_TARBALL"
        OAUTH2_SHA="$OAUTH2_TMP_DIR/oauth2-proxy-v${OAUTH2_VERSION}.linux-${OAUTH2_ARCH}-sha256sum.txt"

        echo "  > Downloading OAuth2 Proxy $OAUTH2_VERSION ($OAUTH2_ARCH)..."
        curl -L -f -o "$OAUTH2_TAR" "$OAUTH2_URL" || true
        curl -L -f -o "$OAUTH2_SHA" "$OAUTH2_SHA_URL" || true

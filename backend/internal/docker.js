...
class DockerService {
    ...
    async init() {
        try {
            ...
            // 1. Local Docker Socket (Default)
            // Unless explicitly disabled with DOCKER_SKIP_LOCAL=true? 
            // User didn't ask for skip, so we assume local is always included unless overriden entirely?
            // Let's assume standard behavior: DOCKER_HOSTS adds *additional* or *replacement*?
            // Usually DOCKER_HOSTS replaces. But here we might want additive.
            // Let's make it additive to local socket if socket exists.

            // Actually, typical use: default to socket. If DOCKER_HOSTS is set, use that list.
            // If user wants local socket in DOCKER_HOSTS, they can add 'unix:///var/run/docker.sock'.

...
const internalNginx = {
	...
	configure: async (model, host_type, host, options = {}) => {
		const firewallPolicyId = host_type === "proxy_host" ? Number(host.firewall_policy_id) : 0;
		if (firewallPolicyId > 0 && !options.skip_firewall_policy_lock) {
			// Proxy configuration can also be regenerated by access lists, maintenance,
			// Git deploy and AI operations. Re-check under the policy lock so a host
			// snapshot cannot recreate a reference after that policy was removed.
			const { withPolicyLock } = await import("./firewall-policy.js");
			return await withPolicyLock(firewallPolicyId, async () => {
				const currentHost = await model.query().findById(host.id);
				if (!currentHost) return {};
				const currentPolicyId = Number(currentHost.firewall_policy_id) || null;
				if (currentPolicyId !== firewallPolicyId) {
					return await internalNginx.configure(
						model,
						host_type,
						{ ...host, firewall_policy_id: currentPolicyId },
						options,
					);
				}
				return await internalNginx.configure(model, host_type, host, {
					...options,

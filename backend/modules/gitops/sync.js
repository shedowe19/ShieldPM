...
const commitAndPush = async (message) => {
	...
	try {
		...
		const sha = await git.commit({ fs, dir: GITOPS_DIR, message: message || `ShieldPM configuration backup - ${new Date().toISOString()}`, author: { name: "ShieldPM GitOps", email: "gitops@shieldpm.local" } });
		const remotes = await git.listRemotes({ fs, dir: GITOPS_DIR });
		const hasOrigin = remotes.some((r) => r.remote === "origin");
		if (!hasOrigin && config.repository_url) await git.addRemote({ fs, dir: GITOPS_DIR, remote: "origin", url: config.repository_url });
		...
		if (config.repository_url) await git.push({ fs, http, dir: GITOPS_DIR, remote: "origin", ref: config.branch || "main", ...getAuth(config) });
		await settingModel.query().where("id", "gitops-config").patch({ meta: { ...config, last_sync: new Date().toISOString(), last_error: null } });
		...
		await settingModel.query().where("id", "gitops-config").patch({ meta: { ...config, last_error: errorMessage } });
...
const pull = async () => {
	if (isDemoMode()) throw new errs.AuthError("GitOps is disabled in Demo Mode");
	const config = await getConfigInternal();
	if (!config.enabled || !config.repository_url) return { success: false, message: "GitOps is not enabled or repository not configured" };
	try {
		await initRepo();
		const remotes = await git.listRemotes({ fs, dir: GITOPS_DIR });
		if (!remotes.some((r) => r.remote === "origin")) await git.addRemote({ fs, dir: GITOPS_DIR, remote: "origin", url: config.repository_url });
		await git.pull({ fs, http, dir: GITOPS_DIR, ref: config.branch || "main", singleBranch: true, author: { name: "ShieldPM GitOps", email: "gitops@shieldpm.local" }, ...getAuth(config) });
		await settingModel.query().where("id", "gitops-config").patch({ meta: { ...config, last_sync: new Date().toISOString(), last_error: null } });
...
const getHistory = async (limit = 20) => {
	try {
		...
		return commits.map((commit) => ({ sha: commit.oid, message: commit.commit.message, author: commit.commit.author.name, date: new Date(commit.commit.author.timestamp * 1000).toISOString() }));
	} catch (err) {
				...
				try {
					...
					if (data && typeof data === 'object') {
						...
						if (existingId) {
							...
							if (existing && !options.overwrite) { skipped++; return; }
						}
						itemData.is_deleted = 0;
						if (options.overwrite && existingId) {
							if (relationGraph) await modelClass.query().upsertGraph(itemData, { insertMissing: true, relate: true, update: true, noDelete: false });
							else {
								const existing = await modelClass.query().findById(existingId);
							...
							const newRow = relationGraph ? await modelClass.query().insertGraph(itemData) : await modelClass.query().insert(itemData);
							if (itemData.id) importedIds.push(itemData.id); else if (newRow?.id) importedIds.push(newRow.id);
						}
						imported++;
					}
				} catch (err) {
...
const revertToCommit = async (access, sha) => {
	...
	try {
		...
		if (importResult.success) {
			setTimeout(() => {
				try { process.kill(1, 'SIGTERM'); } catch (e) { logger.error('Failed to kill PID 1:', e); process.exit(1); }
			}, 1000);

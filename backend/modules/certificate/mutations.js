...
const addCreatedAuditLog = async (access, certificateId, meta) => {
	await internalAuditLog.add(access, { action: "created", object_type: "certificate", object_id: certificateId, meta });
};
...
const writeCustomCert = async (certificate) => {
	...
	if (certificate.provider === "letsencrypt" || certificate.provider === "internal") throw new Error("Refusing to write certbot/internal certs here");
	let certData = certificate.meta.certificate;
	if (typeof certificate.meta.intermediate_certificate !== "undefined") certData = `${certData}\n${certificate.meta.intermediate_certificate}`;
...
const create = async (access, data) => {
	...
	if (thisData.provider === "letsencrypt" || thisData.provider === "internal") thisData.nice_name = thisData.domain_names.join(", ");
	...
	try {
		if (certificate.provider === "letsencrypt") {
			...
			const savedRow = await certificateModel.query().patchAndFetchById(certificate.id, { expires_on: dayjs.unix(certInfo.dates.to).format("YYYY-MM-DD HH:mm:ss") }).then(utils.omitRow(omissions()));
...
const update = async (access, data) => {
	...
	if (row.id !== thisData.id) throw new error.InternalValidationError(`Certificate could not be updated, IDs do not match: ${row.id} !== ${thisData.id}`);
	const savedRow = await certificateModel.query().patchAndFetchById(row.id, thisData).then(utils.omitRow(omissions()));
	...
	await internalAuditLog.add(access, { action: "updated", object_type: "certificate", object_id: row.id, meta: _.omit(thisData, ["expires_on"]) });
	internalGitOps.triggerAutoPush("certificate");
	return savedRow;
};
...
const remove = async (access, data) => {
	...
	await internalAuditLog.add(access, { action: "deleted", object_type: "certificate", object_id: row.id, meta: _.omit(row, omissions()) });
...
const createQuickCertificate = async (access, data) => create(access, { provider: "letsencrypt", domain_names: data.domain_names, meta: data.meta });
...
const upload = async (access, data) => {
	...
	if (typeof validations.certificate === "undefined") throw new error.ValidationError("Certificate file was not provided");
	_.map(data.files, (file, name) => {
		if (allowedSslFiles.indexOf(name) !== -1) row.meta[name] = file.data.toString();
	});
	const certificate = await update(access, { id: data.id, expires_on: dayjs.unix(validations.certificate.dates.to).format("YYYY-MM-DD HH:mm:ss"), domain_names: [validations.certificate.cn], meta: _.clone(row.meta) });
	certificate.meta = row.meta;
	await writeCustomCert(certificate);
	return _.pick(row.meta, allowedSslFiles);
};

export { addCreatedAuditLog, create, createQuickCertificate, getLiveCertPath, remove, requestCertbot, requestCertbotWithDnsChallenge, revokeCertbot, update, upload, writeCustomCert };

...
const create = async (access, data) => {
	...
	await userModel.transaction(async (trx) => {
		...
		await userPermissionModel.query(trx).insert({ user_id: user.id, visibility: "user", access_lists: "manage", certificates: "manage", proxy_hosts: "manage", redirection_hosts: "manage", streams: "manage", dead_hosts: "manage", cloudflared_tunnels: "manage", analytics: "view" });
	});
...
const update = async (access, data) => {
	...
	if (user.id !== data.id) throw new errs.InternalValidationError(`User could not be updated, IDs do not match: ${user.id} !== ${data.id}`);
	let avatarType = data.avatar_type || user.avatar_type || "gravatar";
	let avatarValue = data.avatar_value === undefined ? user.avatar_value : data.avatar_value;
	...
	await auditLogService.add(access, { action: "updated", object_type: "user", object_id: user.id, meta: { ...data, id: user.id, name: user.name } });
	return user;
};
...
const remove = async (access, data) => {
	...
	await auditLogService.add(access, { action: "deleted", object_type: "user", object_id: user.id, meta: _.omit(user, omissions()) });
	return true;
};
...
const setPassword = async (access, data) => {
	...
	if (user.id === access.token.getUserId(0)) {
		if (typeof data.current === "undefined" || !data.current) throw new errs.ValidationError("Current password was not supplied");
	...
	if (existing_auth) {
		await authModel.query().where("user_id", user.id).andWhere("type", data.type).patch({ type: data.type, secret: data.secret });
	} else {
		await authModel.query().insert({ user_id: user.id, type: data.type, secret: data.secret, meta: {} });
	}
	await auditLogService.add(access, { action: "updated", object_type: "user", object_id: user.id, meta: { name: user.name, password_changed: true, auth_type: data.type } });
	return true;
};
...
const setPermissions = async (access, data) => {
	...
	let permissions;
	if (existing_auth) {
		permissions = await userPermissionModel.query().where("user_id", user.id).patchAndFetchById(existing_auth.id, _.assign({ user_id: user.id }, data));
	} else {
		permissions = await userPermissionModel.query().insertAndFetch(_.assign({ user_id: user.id }, data));
	}
	await auditLogService.add(access, { action: "updated", object_type: "user", object_id: user.id, meta: { name: user.name, permissions } });
	return true;
};

 ...
 */
router.all(jwtdecode());

router.get('/', async (req, res, next) => {
    try {
        const tunnels = await CloudflaredTunnel.query().where('owner_user_id', res.locals.access.token.getUserId(1)).andWhere('is_deleted', 0).orderBy('name', 'ASC');

...
router.get("/summary", async (req, res) => {
  try {
    ...
    const stats = (await AnalyticCount.knex().from("analytic_count").where("timestamp", ">=", start).andWhere("timestamp", "<=", end).sum("request_count as count").sum("bytes_sent as bytes").sum("status_code_2xx as s2xx").sum("status_code_3xx as s3xx").sum("status_code_4xx as s4xx").sum("status_code_5xx as s5xx").first()) || {};
    const defaults = {
      count: 0,
      bytes: 0,
      s2xx: 0,
...
router.get("/series", async (req, res) => {
  try {
    ...
    const data = await AnalyticCount.query().where("timestamp", ">=", start).andWhere("timestamp", "<=", end).orderBy("timestamp", "asc");
  ...
  try {
    ...
    if (isSqlite()) {
      ...
      if (ioResult.rows[0]) {
        ...
        stats.io.writes = Number.parseInt(ioResult.rows[0].tup_inserted || 0, 10) + Number.parseInt(ioResult.rows[0].tup_updated || 0, 10) + Number.parseInt(ioResult.rows[0].tup_deleted || 0, 10);
      }
    }
    res.json(stats);
  } catch (err) {

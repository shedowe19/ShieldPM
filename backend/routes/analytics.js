...
router.get("/summary", async (req, res) => {
    try {
        ...
        res.json({
            ...
            status_2xx: parseInt(stats.s2xx || 0, 10),
            status_3xx: parseInt(stats.s3xx || 0, 10),
            status_4xx: parseInt(stats.s4xx || 0, 10),
            status_5xx: parseInt(stats.s5xx || 0, 10)
        });
    } catch (err) {
...
router.get("/series", async (req, res) => {
    try {
        ...
        for (const row of data) {
            ...
            g.bytes += parseInt(row.bytes_sent, 10); // sqlite might return string for bigint

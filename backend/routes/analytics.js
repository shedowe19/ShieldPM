...
router.get("/summary", async (req, res) => {
	try {
		...
		res.json({
			count: Number.parseInt(stats.count || 0, 10),
			bytes: Number.parseInt(stats.bytes || 0, 10),
			status_2xx: Number.parseInt(stats.s2xx || 0, 10),
			status_3xx: Number.parseInt(stats.s3xx || 0, 10),
			status_4xx: Number.parseInt(stats.s4xx || 0, 10),
			status_5xx: Number.parseInt(stats.s5xx || 0, 10),
		});
	} catch (err) {

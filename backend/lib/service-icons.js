/**
 * Service Icon Detection Library
 ...
 * @property {string} [hostname] - Optional hostname pattern for more specific matching
 * @property {string} name - Service identifier (used for CDN URL)
 ...
 */
export function detectService(port, hostname = "") {
	const lowerHostname = hostname.toLowerCase();

	// First try exact match with hostname (more specific)
	const hostnameMatch = SERVICES.find((s) => s.port === port && s.hostname && lowerHostname.includes(s.hostname));
 ...
 * @returns {string}
 */
export function getIconUrl(serviceName) {
	return `${ICON_CDN_BASE}/${serviceName}.svg`;
}

/**
 * Get all available services (for autocomplete/picker)
 * @returns {Array<{name: string, displayName: string, iconUrl: string}>}
 */
...
export default { detectService, getIconUrl, getAllServices, SERVICES, ICON_CDN_BASE };

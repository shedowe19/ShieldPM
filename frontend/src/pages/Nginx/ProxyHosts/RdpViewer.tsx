...
const SCANCODE_MAP: Record<string, number> = {
	...
	KeyF: 0x21, KeyG: 0x22, KeyH: 0x23, KeyJ: 0x24, KeyK: 0x25,
	KeyL: 0x26, Semicolon: 0x27, Quote: 0x28,
	...
	Delete: 0x53, ControlRight: 0x9d, AltRight: 0xb8,
	MetaLeft: 0xdb, MetaRight: 0xdc, ContextMenu: 0xdd,
...
const VKEY_ROWS: VKey[][] = [
	...
	[
		...
		},
		{
			label: "Ctrl+C",
			action: [
				[SC_CTRL, true], [SCANCODE_MAP.KeyC, true],
				[SCANCODE_MAP.KeyC, false], [SC_CTRL, false],
			],
		},
		{
			label: "Ctrl+V",
			action: [
				[SC_CTRL, true], [SCANCODE_MAP.KeyV, true],
				[SCANCODE_MAP.KeyV, false], [SC_CTRL, false],
			],
		},
		{
			label: "Ctrl+X",
			action: [
				[SC_CTRL, true], [SCANCODE_MAP.KeyX, true],
				[SCANCODE_MAP.KeyX, false], [SC_CTRL, false],
			],
		},
		{
			label: "Ctrl+Z",
			action: [
				[SC_CTRL, true], [SCANCODE_MAP.KeyZ, true],
				[SCANCODE_MAP.KeyZ, false], [SC_CTRL, false],
			],
		},
		{
			label: "Ctrl+A",
			action: [
				[SC_CTRL, true], [SCANCODE_MAP.KeyA, true],
				[SCANCODE_MAP.KeyA, false], [SC_CTRL, false],
			],
		},
		{
			label: "Alt+Tab",
			action: [
				[SC_ALT, true], [SCANCODE_MAP.Tab, true],
				[SCANCODE_MAP.Tab, false], [SC_ALT, false],
			],
		},
		{
			label: "Alt+F4",
			action: [
				[SC_ALT, true], [SCANCODE_MAP.F4, true],
				[SCANCODE_MAP.F4, false], [SC_ALT, false],
			],
		},
	],
];
...
export default function RdpViewer({ hostId, open, onClose }: RdpViewerProps) {
	...
	const connect = useCallback(() => {
		...
		ws.onmessage = (evt) => {
			try { handleMessage(JSON.parse(evt.data)); } catch (_e) { /* ignore */ }
		};
		ws.onclose = () => {
			wsRef.current = null;
	...
	// handleMessage is stable (defined below with useCallback + no deps that change)
	...
	const handleMessage = useCallback((msg: Record<string, unknown>) => {
		switch (msg.type) {
			case "status": {
				...
				if (s === "disconnected") { setStatus("disconnected"); wsRef.current = null; }
				break;
			}
			case "size":
	...
	// renderBitmap is defined below and is stable
	// eslint-disable-next-line react-hooks/exhaustive-deps
	}, []);
	...
	const renderBitmap = useCallback((msg: Record<string, unknown>) => {
		...
		const { destLeft, destTop, width, height, bitsPerPixel, data } = msg as {
			destLeft: number; destTop: number;
			width: number; height: number;
			bitsPerPixel: number; data: string;
		};
		if (!data) return;
		try {
		...
		} catch (_e) { /* ignore rendering errors */ }
	}, []);
	...
	const handleVKey = useCallback((vkey: VKey) => {
		if (vkey.modifier) {
			// Toggle modifier
	...
	return (
		<Dialog open={open} onOpenChange={(o) => !o && onClose()}>
			<DialogContent className="max-w-[98vw] max-h-[98vh] w-full h-full p-0 gap-0 flex flex-col overflow-hidden">
				...
				{/* ── Header bar ── */}
				<DialogHeader ref={headerRef} className="px-3 py-1.5 border-b flex-shrink-0">
					<DialogTitle className="flex items-center gap-2 text-sm">
						...
						{errorMsg && (
							<span className="text-xs text-destructive font-normal truncate max-w-xs" title={errorMsg}>
								— {errorMsg}
							</span>
						)}
				...
				<div
					...
					// Allow canvas to capture keyboard events when clicked
					onKeyDown={onKeyDown}
					onKeyUp={onKeyUp}
					tabIndex={-1}
				>
					...
					{(status === "connecting") && (
						<div className="absolute inset-0 flex items-center justify-center bg-black/70 pointer-events-none">
							<div className="text-white text-center">
								...
								<p className="text-sm"><T id="rdp.connecting" /></p>
							</div>
						</div>
					)}
				...
				{showVKB && (
					<div
						ref={vkbRef}
						className="flex-shrink-0 border-t bg-background overflow-x-auto"
					>
								...
								{row.map((vkey, keyIdx) => {
									...
									return (
										<button
											...
											onTouchStart={(e) => { e.preventDefault(); handleVKey(vkey); }}

    ...
    return (
        <Dialog open={visible} onOpenChange={(open) => !open && remove()}>
            <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto">
                ...
                {(isLoading && isEditing) ? <Loading noLogo /> : (
                    <Formik
                        enableReinitialize
                        initialValues={{
                            name: data?.name || "",
                    ...
                    >
                        {({ values, setFieldValue }: any) => (
                            <Form className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="provider"><T id="ddns-providers.provider" /></Label>
                                    <Field name="provider">
                                        {({ field }: any) => (
                                            <Select value={field.value} onValueChange={(val) => setFieldValue("provider", val)}>
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="domainsStr"><T id="ddns-providers.domains" /></Label>
                                    <Field name="domainsStr">
                                        {({ field }: any) => <Input {...field} id="domainsStr" placeholder="sub.example.com, sub2.example.com" />}
                                    </Field>
                                    <div className="text-xs text-muted-foreground">Comma separated</div>
                                </div>

                                <div className="space-y-2">
                                    <Label htmlFor="ip_ver"><T id="ddns-providers.ip-ver" /></Label>
                                    <Field name="ip_ver">
                                        {({ field }: any) => (
                                            <Select value={field.value} onValueChange={(val) => setFieldValue("ip_ver", val)}>
                                                <SelectTrigger id="ip_ver">
                                                    <SelectValue />
                                                </SelectTrigger>
                                                <SelectContent>
                                                    <SelectItem value="dual"><T id="ddns-providers.ip-ver.dual" /></SelectItem>
                                                    <SelectItem value="v4"><T id="ddns-providers.ip-ver.v4" /></SelectItem>
                                                    <SelectItem value="v6"><T id="ddns-providers.ip-ver.v6" /></SelectItem>
                                                </SelectContent>
                                            </Select>
                                        )}
                                    </Field>
                                ...
                                <div className="border border-dashed p-4 rounded-md space-y-4">
                                    <h3 className="font-medium text-sm"><T id="ddns-providers.config" /></h3>

                                    {values.provider === "cloudflare" && (
                                        <>
                                            <div className="space-y-2">
                                                <Label htmlFor="cloudfare_token"><T id="ddns-providers.cloudfare_token" /></Label>
                                                <Field name="cloudfare_token">
                                                    {({ field }: any) => <Input {...field} type="password" id="cloudfare_token" />}
                                                </Field>
                                            </div>
                                            <div className="space-y-2">
                                                <Label htmlFor="cloudfare_zone_id"><T id="ddns-providers.cloudfare_zone_id" /></Label>
                                    ...
                                    {values.provider === "duckdns" && (
                                        <div className="space-y-2">
                                            <Label htmlFor="duckdns_token"><T id="ddns-providers.duckdns_token" /></Label>
                                            <Field name="duckdns_token">
                                                {({ field }: any) => <Input {...field} type="password" id="duckdns_token" />}
                                            </Field>
                                        </div>
                                    )}
                                    ...
                                    {values.provider === "custom" && (
                                        <div className="space-y-2">
                                            <Label htmlFor="custom_url"><T id="ddns-providers.custom_url" /></Label>
                                            <Field name="custom_url">
                                                {({ field }: any) => <Input {...field} id="custom_url" placeholder={intl.formatMessage({ id: "ddns-providers.custom_url.placeholder" })} />}
                                            </Field>
                                            <div className="text-xs text-muted-foreground">
                                                Supports &#123;IP&#125;, &#123;IPv4&#125;, &#123;IPv6&#125;, &#123;DOMAIN&#125;
                                            </div>
                                        </div>
                                    )}
                                </div>

...
import { type DdnsProvider, createDdnsProvider, updateDdnsProvider, testDdnsProvider } from "src/api/backend";
...
const showDdnsProviderModal = (id?: number) => {
    EasyModal.show(DdnsProviderModal, { id });
};
...
const DdnsProviderModal = EasyModal.create(({ id, visible, remove }: Props) => {
    ...
    // For simplicity, we fetch all and find the one (since we don't have a specific get hook yet)
    // In a real app, use a dedicated hook like useDdnsProvider(id)

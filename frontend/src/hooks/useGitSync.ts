...
import { getGitSyncStatus, triggerGitSync, updateGitSyncConfig, type GitSyncConfig, type GitSyncResult, type GitSyncStatus } from "src/api/backend/gitSync";
...
export function useUpdateGitSyncConfig() {
    ...
    return useMutation<GitSyncStatus, Error, { hostId: number; config: GitSyncConfig }>({
        mutationFn: ({ hostId, config }: { hostId: number; config: GitSyncConfig }) => updateGitSyncConfig(hostId, config),

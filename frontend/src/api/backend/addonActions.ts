...
export async function installAddon(
	id: string,
	manifest: RegistryAddon,
): Promise<{ installed: boolean; binaryPath: string | null }> {
...
export async function updateAddon(
	id: string,
): Promise<{ updated: boolean; from?: string; to: string; reason?: string }> {
	return await api.post({ url: `/addons/${id}/update` });
}

export async function updateAllAddons(): Promise<
	Array<{ id: string; updated: boolean; from?: string; to?: string; error?: string }>
> {

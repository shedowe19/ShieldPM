...
This page mirrors the current repository `compose.yaml` for ShieldPM **4.3.2** on the `develop` branch.

Use [Installation](Installation) for the quick start and this page when you need the full option set, optional services, WireGuard capabilities, OpenAppSec services, CrowdSec, GeoIP updates, Caddy redirector, and database examples.

> [!NOTE]
> The current development compose file uses `ghcr.io/shedowe19/shieldpm:develop`. For release deployments you may pin a release tag or switch to `ghcr.io/shedowe19/shieldpm:latest` intentionally.
...
[🏠 Home](Home) | [📖 Configuration](Configuration) | [🐞 Report a Bug](https://github.com/shedowe19/ShieldPM/issues)

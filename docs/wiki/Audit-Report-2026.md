...
**Date:** 2026-01-15  
**Scope:** Complete recursive audit of all functional units (historical report, refreshed for ShieldPM v4.3.2 documentation context)
...
| ------------------------------------------------- | ---------------------------------------- |
| [app.js](../../backend/app.js)                    | `csrf-csrf` Double Submit Cookie pattern |
...
> [!NOTE]
>
> 1. **AI internals**: AI logic is now split across `backend/internal/ai/` provider/executor/tools modules while `backend/internal/ai.js` remains a configuration/service entrypoint.
> 2. **Certificate logic**: Certificate handling remains centralized in `backend/internal/certificate.js`; keep certbot and model responsibilities separated when extending it.
> 3. **Type Safety**: Backend and frontend use TypeScript/JSDoc-aware checks plus Vitest coverage where applicable.
> 4. **Docker Auto-Discovery**: Keep advanced-config directive whitelisting and service-account behavior stable when adding labels.
...
| **Config Validation**           | `internalNginx.test()` exists, but `nginx -t` is currently skipped before reload for speed. Template errors can still break Nginx. |
| **Error Recovery**              | Startup scripts attempt to disable broken proxy-host configs with missing certificates so the backend can recover.                 |

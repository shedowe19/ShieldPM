...
- `modals/account-lazy.ts` lädt `UserModal.tsx` und `ChangePasswordModal.tsx` erst bei einer Kontoaktion; `modals/lazy.ts` enthält weiterhin generische Dialog-Wrapper, wird aber von keiner Produktionsroute mehr statisch importiert. Die Wrapper speichern je Import-Promise; bei Ladefehlern verwerfen sie es und nutzen die vorhandene Fehler-Toast-Schicht. `SiteHeader.tsx` verwendet den kleinen Konto-Loader, damit die global eingebundene Kopfzeile nicht die dynamischen Import-Metadaten aller anderen Dialoge enthält. `Users/TableWrapper.tsx` verwendet daneben `Users/lazy.ts`, der die Berechtigungs-, Passwort- und Löschdialoge erst bei Benutzeraktionen importiert. Dadurch referenziert die Users-Route den allgemeinen `modals/lazy.ts`-Chunk nicht mehr statisch. `ProxyHosts/Table.tsx` importiert den `AccessListFormatter` direkt statt über `src/components`, damit dessen Modal-Loader nicht den Start-Chunk erreicht. `Certificates/lazy.ts` bündelt alle von Zertifikatsseiten ausgelösten Dialogaktionen, sodass diese Routen nicht den allgemeinen `modals/lazy.ts`-Chunk laden. `Access/lazy.ts` kapselt die drei von `Access/TableWrapper.tsx` ausgelösten Dialoge; die Access-Route enthält dadurch nur deren Import-Metadaten und importiert den allgemeinen Loader nicht mehr. `AuditLog/lazy.ts` kapselt den Event-Details-Dialog, sodass die Audit-Log-Route nur dessen Import-Metadaten enthält und den allgemeinen Loader nicht lädt. `Nginx/CloudflaredTunnels.lazy.ts` kapselt den Hilfe-Dialog der Cloudflared-Route, damit diese den gemeinsamen `modals/lazy.ts`-Chunk nicht mehr statisch lädt. `Nginx/TorOnionServices.lazy.ts` kapselt den Hilfe-Dialog der Tor-Route, damit diese den gemeinsamen `modals/lazy.ts`-Chunk nicht mehr statisch lädt. `Nginx/WireguardTunnels.lazy.ts` kapselt den Hilfe-Dialog der WireGuard-Route, damit diese den gemeinsamen `modals/lazy.ts`-Chunk nicht mehr statisch lädt. `ProxyHosts/TableWrapper.tsx`, `AccessListformatter.tsx`, `DashboardNotesWidget.tsx`, `AuditLog/TableWrapper.tsx`, `Certificates/Table.tsx`, `Certificates/TableWrapper.tsx`, `Nginx/RedirectionHosts/TableWrapper.tsx` und `Nginx/DdnsProviders/TableWrapper.tsx` verwenden dafür jeweils route- oder featureeigene Loader; die aktionsgebundenen Dialogimporte bleiben damit von ihren initialen Routen-Chunks getrennt.
- Das ungenutzte exportierende Modal-Barrel `modals/index.ts` existiert nicht mehr. Ein Regressionstest verhindert seine Wiedereinführung, damit künftige statische Importe nicht versehentlich wieder mehrere Dialogmodule koppeln.

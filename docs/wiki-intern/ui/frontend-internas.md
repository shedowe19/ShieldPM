...
- `context/AuthContext.tsx` leert vor einem direkten Passwort-Login oder einem beanspruchten OIDC-Token den gemeinsamen React-Query-Cache und übernimmt den Token anschließend in `AuthStore` sowie den Auth-Status. Der OIDC-Login aktualisiert damit den angemeldeten UI-Zustand ohne Dokument-Reload.

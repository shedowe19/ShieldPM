...
- Die Vite-Chunk-Regeln bündeln nur stabile React-, Daten- und Hilfsbibliotheken explizit. Radix-, Icon- und Motion-Module werden nicht mehr pauschal in `vendor-ui` zusammengezogen, weil eine einzige dauerhaft benötigte UI-Abhängigkeit sonst alle route-spezifischen Controls beim Start nachlädt. Der gemessene anfängliche Preload aus Entry und `vendor-ui` sank dadurch von 402,38 kB (125,43 kB gzip) auf 212,14 kB (68,10 kB gzip); die gesparten 190,24 kB (57,33 kB gzip) bleiben an bedarfsabhängige Route-Chunks gebunden. `vite.config.test.ts` sichert, dass diese UI-Pakete nicht erneut einem gemeinsamen App-Shell-Chunk zugewiesen werden.

...
- `components/LazyCodeEditor.tsx` kapselt den Code-Editor zusätzlich hinter einem renderabhängigen `React.lazy()`-Import. `LocationsFields` fordert den Editor daher erst beim Öffnen der erweiterten Standortoptionen an; `DefaultSite` erst nach Wahl der HTML-Variante. Der 1,29-kB-Loader übernimmt währenddessen eine höhenstabile Placeholder-Fläche, während der eigentliche Editor-Chunk (813,49 kB / 286,53 kB gzip) nur bei einer dieser Nutzeraktionen geladen wird. `Form/code-editor-imports.test.ts` sichert die dynamische Importgrenze und beide Konsumenten.

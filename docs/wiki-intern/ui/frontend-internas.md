...
- `components/LocaleRefreshBoundary.tsx` aktualisiert nach einem Sprachwechsel den `RawIntlProvider`, ohne den UI-Unterbaum neu zu mounten. `LocaleContext` stellt dafür eine Locale-Revision bereit, die `T`-Konsumenten gezielt neu rendert; offene Formulare, Modals, Fokus und Scrollposition bleiben erhalten. Die gemeinsame `queryClient`-Instanz und bereits geladene React-Query-Daten bleiben ebenfalls erhalten.

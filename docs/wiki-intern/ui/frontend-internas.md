...
- `context/AuthContext.tsx` leert vor einem direkten Passwort-Login, einem beanspruchten OIDC-Token oder einem erfolgreich verifizierten 2FA-Token den gemeinsamen React-Query-Cache und übernimmt den Token anschließend in `AuthStore` sowie den Auth-Status. Bei einem Benutzerwechsel wird der Cache ebenfalls geleert und der Session-Unterbaum per versioniertem React-Fragment neu gemountet. Diese Login- und Wechsel-Flows aktualisieren damit den angemeldeten UI-Zustand ohne Dokument-Reload.
...
- `modals/lazy.ts` lädt `ProxyHostModal.tsx`, `AccessListModal.tsx`, `UserModal.tsx`, `ChangePasswordModal.tsx`, `CustomCertificateModal.tsx`, `DNSCertificateModal.tsx`, `HTTPCertificateModal.tsx`, `InternalCertificateModal.tsx`, `RenewCertificateModal.tsx`, `PermissionsModal.tsx`, `SetPasswordModal.tsx`, `StreamModal.tsx`, `DashboardNoteModal.tsx`, `DeadHostModal.tsx`, `DdnsProviderModal.tsx`, `DeleteConfirmModal.tsx`, `EventDetailsModal.tsx`, `HelpModal.tsx` sowie `RedirectionHostModal.tsx` erst bei einer Anforderung. Die Wrapper speichern je Import-Promise; bei Ladefehlern verwerfen sie es und nutzen die vorhandene Fehler-Toast-Schicht. `ProxyHosts/TableWrapper.tsx`, `Access/TableWrapper.tsx` (einschließlich Hilfe und Löschbestätigung), `AccessListformatter.tsx`, `Users/TableWrapper.tsx`, `SiteHeader.tsx`, `DashboardNotesWidget.tsx`, `AuditLog/TableWrapper.tsx`, `Certificates/Table.tsx`, `Certificates/TableWrapper.tsx`, `Nginx/RedirectionHosts/TableWrapper.tsx`, `Nginx/Streams/TableWrapper.tsx`, `Nginx/DdnsProviders/TableWrapper.tsx`, `Nginx/DeadHosts/TableWrapper.tsx`, `Nginx/CloudflaredTunnels.tsx`, `Nginx/TorOnionServices.tsx` und `Nginx/WireguardTunnels.tsx` verwenden diese Wrapper statt des `modals`-Barrels, damit die Dialoge nicht über gemeinsame Barrel-Abhängigkeiten bereits mit dem jeweiligen Routen-Chunk geladen werden.

...
- `api/queryClient.ts` erzeugt genau einen React-Query-Client. Sowohl der Provider in `App.tsx` als auch die zentrale Antwortverarbeitung nutzen diese Instanz; ein 401 leert damit den tatsächlich sichtbaren Cache vor einer möglichen neuen Anmeldung. Normale Konfigurationsabfragen erben zentral `staleTime: 60_000` und `refetchOnWindowFocus: false`; Live-Abfragen wie Health setzen ihre kürzeren Polling-Regeln weiterhin explizit. Mutationen invalidieren die betroffenen Cache-Keys unverändert.

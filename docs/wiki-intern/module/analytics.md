...
- Das Dashboard zeigt für Benutzer mit `analytics:view` oder `analytics:manage` die fünf Proxy-Hosts mit den meisten Requests der letzten 24 Stunden. Die serverseitige Richtlinie `analytics:list` akzeptiert dieselben View-/Manage-Berechtigungen; `GET /api/analytics/top-hosts` aggregiert ausschließlich nicht gelöschte, hostgebundene Zähler. Jeder Host verlinkt direkt auf seine vorhandene 24-Stunden-Analytics-Ansicht.

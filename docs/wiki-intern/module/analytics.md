...
- Das Dashboard zeigt für Benutzer mit `analytics:view` oder `analytics:manage` getrennt die fünf Proxy-Hosts mit den meisten Requests sowie die fünf Hosts mit den meisten 4xx- beziehungsweise 5xx-Antworten der letzten 24 Stunden. Die serverseitige Richtlinie `analytics:list` akzeptiert dieselben View-/Manage-Berechtigungen; `GET /api/analytics/top-hosts` aggregiert ausschließlich nicht gelöschte, hostgebundene Zähler und liefert immer Request-, 4xx- sowie 5xx-Zähler. Standardmäßig ordnet der Endpunkt nach Requests; `sort=client_errors` und `sort=server_errors` wählen ausschließlich die fest definierte 4xx- beziehungsweise 5xx-Ordnung. Die 4xx-Rangliste macht fehlgeschlagene oder zurückgewiesene Client-Anfragen unmittelbar sichtbar; jeder Host verlinkt direkt auf seine vorhandene 24-Stunden-Analytics-Ansicht.

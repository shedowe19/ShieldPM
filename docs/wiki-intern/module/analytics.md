...
- Das Dashboard zeigt für Benutzer mit `analytics:view` oder `analytics:manage` getrennt die fünf Proxy-Hosts mit den meisten Requests sowie die fünf Hosts mit den meisten 5xx-Antworten der letzten 24 Stunden. Die serverseitige Richtlinie `analytics:list` akzeptiert dieselben View-/Manage-Berechtigungen; `GET /api/analytics/top-hosts` aggregiert ausschließlich nicht gelöschte, hostgebundene Zähler und liefert immer Request- sowie 5xx-Zähler. Standardmäßig ordnet der Endpunkt nach Requests; `sort=server_errors` wählt ausschließlich die fest definierte 5xx-Ordnung. Jeder Host verlinkt direkt auf seine vorhandene 24-Stunden-Analytics-Ansicht.

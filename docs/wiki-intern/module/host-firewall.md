...
- Feed-Dateien und die zentrale Nginx-Konfiguration werden atomar geschrieben. Jeder Read-Render-Replace-Zyklus der gemeinsamen Map ist zusätzlich global serialisiert, sodass ein älterer Policy-Snapshot keinen neueren Map-Stand wieder überschreiben kann. Ein vollständiger Satz neuer Cache-Dateien wird erst ersetzt, wenn jede konfigurierte Quelle entweder erfolgreich aktualisiert wurde oder weiterhin einen gültigen Cache besitzt. Bei Abruffehlern bleibt die letzte gültige Version wirksam; fehlt der einzige gültige Cache, wird der persistierte inaktive Status noch im selben Scheduler-Lauf in die Nginx-Map geschrieben und neu geladen. Dasselbe gilt für einen fehlgeschlagenen manuellen Abruf: Er meldet den Fehler an die API, veröffentlicht aber zuvor die inaktive Map. Auch ein Überschreiten des kombinierten CIDR-Limits markiert alle Feed-Caches der Policy explizit als unvollständig, sodass nie nur die manuellen CIDRs einer scheinbar aktiven Policy erzwungen werden. Eine neu angelegte, cachelose Feed-Policy bleibt bis zum ersten erfolgreichen Abruf deaktiviert.
...
Bei aktivem GitOps werden Policy-Definitionen unter `firewall-policies/` sowie die `firewall_policy_id`-Zuordnung der Proxy-Hosts exportiert und beim Restore **vor** den Proxy-Hosts importiert. Jede Policy-Datei benötigt dafür eine positive stabile ID. Eine bestehende lokale ID darf bei einem nicht überschreibenden Restore nur verwendet werden, wenn die deklarative Policy vollständig übereinstimmt; eine abweichende ID oder ein Host-Verweis ohne passende Policy derselben Revision wird explizit abgelehnt. Bei einem überschreibenden Restore setzt eine ältere Proxy-Host-Datei ohne dieses damals noch nicht existierende Feld eine vorhandene Policy-Zuordnung ausdrücklich auf `null`; ein Rollback kann keine nicht deklarierte Sperre behalten. Jede importierte Policy durchläuft dieselbe Typ-, CIDR-, Länder-, Feed- und Aktionsvalidierung wie die API. Laufzeitdaten wie Feed-Status, Fehlertexte, Zeitstempel und die kompilierten CIDR-Dateien werden bewusst nicht versioniert. ShieldPM aktualisiert die jeweils konfigurierten Quellen vor dem Rendern der Proxy-Hosts mit begrenzter Parallelität, bewahrt dabei jedoch einen vorhandenen letzten gültigen Cache bis zum vollständigen Ersatz. Fehlt für eine neue oder geänderte Quelle während des Restores ein gültiger Cache, wird kein Nginx-Reload mit einer leeren Feed-Sperre durchgeführt; die Policy bleibt bis zur erfolgreichen Aktualisierung inaktiv. Policy-Updates, manuelle Abrufe und der Scheduler sind pro Policy serialisiert, damit ein älterer Abruf keine neuere Feed-Konfiguration zurückschreiben kann. Zusätzlich ist der gemeinsame Firewall-Map-Render global serialisiert. Jeder Proxy-Host-Render und jede Konfigurationsentfernung ist pro Host serialisiert und prüft vor dem Schreiben den aktuellen aktiven DB-Status; Access-List-, Maintenance-, Git-Deploy-, Docker-, Zertifikats- und AI-Pfade können deshalb keine gelöschte, deaktivierte oder veraltete Firewall-Zuweisung wiederherstellen. Proxy-Host-Erstellungen und -Änderungen sperren außerdem sowohl die bisherige als auch die gewünschte Policy während Persistenz und Nginx-Rendern. Bei Full Sync bleibt eine Übergangs-Map bestehen, bis alle aktiven Proxy-Host-Konfigurationen ohne veraltete Policy gerendert sind; anschließend erfolgt ein gesperrter Verwendungscheck vor dem endgültigen Entfernen von Policy und Cache. Bei Fehlern an Policies oder abhängigen Hosts unterbleibt dieses Entfernen. Damit kann weder ein paralleler Vorgang noch ein unvollständiger Restore eine Host-Konfiguration mit bereits entfernten Nginx-Maps hinterlassen.
...
- `backend/internal/proxy-host.js` und `backend/internal/nginx.js` — sperren Policy-Zuordnungen sowie jeden Proxy-Host-Render/-Delete gegen parallele Policy- und Lifecycle-Operationen
- `backend/lib/proxy-host-config-lock.js` — pro Host serialisierte Nginx-Render- und Entfernungsoperationen

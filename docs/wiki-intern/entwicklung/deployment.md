...
| `npm-updates.yml`                         | Täglicher Helper für direkte npm-Minor-/Patch-Updates: klont mit vollständiger Git-Historie, startet vom eingefrorenen Yarn-Lockfile, nutzt für jeden Projektbefehl Node-26-gebundenes Yarn über `npx --yes --package yarn@1.22.22 yarn` und stellt `ncu` sowie `license-checker` über npm mit geprüften Binärdateien bereit. Er generiert die Lizenzhinweise fail-closed und erstellt nur nach Backend-/Frontend-Tests sowie Frontend-Build bei Manifest-Änderungen mit dem projektgebundenen Actions-Secret `DEPENDENCY_UPDATES_TOKEN` eine PR. |

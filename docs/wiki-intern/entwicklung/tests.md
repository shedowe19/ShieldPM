...
- `backend/test/migrations/mariadb-interrupted-zstd-runner.spec.js` wird nur mit `SHIELDPM_TEST_MARIADB=1` ausgeführt. Der CI-Job stellt diese Variable und einen echten MariaDB-11.8.3-Service bereit. Der Test erzeugt einen isolierten Datenbanknamen, legt die Zstd-Spalte ohne Migrationseintrag an und überprüft danach über den produktiven Knex-Runner die Registrierung sowie einen wiederholten Lauf.

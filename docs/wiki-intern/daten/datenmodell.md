...
| ProxyHostMonitor      | `proxy_host_monitor.js`       | `proxy_host_monitor`       | Konfiguration und Zustand je Proxy-Host |
| ProxyHostMonitorCheck | `proxy_host_monitor_check.js` | `proxy_host_monitor_check` | Begrenzter Prüfverlauf je Host          |
...
- `ProxyHost` → `ProxyHostMonitor` (1:0..1 über `host_id`, ohne ORM-Relation)
- `ProxyHost` → `ProxyHostMonitorCheck` (1:n über `host_id`, ohne ORM-Relation)

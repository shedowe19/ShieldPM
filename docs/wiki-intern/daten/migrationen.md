...
Die Tests `backend/test/migrations/full-schema.spec.js` und `data-preservation.spec.js` führen diese Abläufe mit SQLite und der echten PostgreSQL-Engine in PGlite aus. Der separate CI-Job `mariadb-migration` startet MariaDB 11.8.3 und führt die vollständige Suite `mariadb-*.spec.js` aus. `mariadb-full-schema.spec.js` erstellt eine frische isolierte Datenbank und lässt über den produktiven `runMigrations()`-Runner jede vorhandene Migrationsdatei von Anfang bis Ende laufen; die registrierten Dateinamen müssen danach exakt dem Migrationsverzeichnis entsprechen. Ein zweiter Runner-Lauf darf keine Migration mehr anwenden. `mariadb-interrupted-zstd-runner.spec.js` ergänzt den Unterbrechungsfall einer bereits vorhandenen Spalte ohne Eintrag in `migrations`. Dabei werden nur die externen Nginx-Aktionen gemockt. Änderungen historischer Migrationen wirken auf noch nicht ausgeführte Upgrades und auf Rollbacks; bereits durch frühere Versionen verlorene Werte lassen sich daraus nicht automatisch rekonstruieren.

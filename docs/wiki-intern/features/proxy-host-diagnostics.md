...
2. TLS verbindet sich mit dem lokalen Nginx-HTTPS-Listener und SNI des Hosts. Die normale Prüfung verwendet die Node-Systemvertrauenskette und prüft Zertifikat, Namen und Laufzeit. Bei einer unbekannten privaten CA oder einem selbstsignierten Zertifikat bleibt das TLS-Ergebnis eine Warnung. Für die lokale Route kann das Backend zusätzlich das zugewiesene Zertifikat anhand der numerischen Zertifikats-ID und seines Providers aus `/data/tls/{certbot/live,internal,custom}/npm-ID/fullchain.pem` laden. Ein weiterer streng verifizierter Verbindungsversuch nutzt genau dieses Zertifikat als lokalen Vertrauensanker, überprüft den Hostnamen und vergleicht den Fingerabdruck des tatsächlich präsentierten Leaf-Zertifikats. Die private Zertifikatsdatei erweitert die allgemeine Vertrauenskette nicht.
3. Die lokale Route sendet `HEAD /` an den eigenen Nginx-Listener mit dem gespeicherten Hostnamen. HTTPS-Requests folgen nur nach erfolgreicher normaler oder lokaler verifizierter TLS-Verbindung. Wird für den lokalen Vertrauensanker ein konfiguriertes Zertifikat benötigt und fehlt dieses, stimmt der Fingerabdruck nicht überein oder passt der Hostname nicht, schlägt die HTTPS-Route ohne HTTP-Anfrage fehl; dasselbe gilt für den WebSocket-Check. HTTP-Weiterleitungen und 401/403 werden ohne Weiterleitung oder Anmeldung erfasst. Bei deaktiviertem HTTP und ohne Zertifikat verwendet die Route den zugehörigen lokalen Unix-Socket.
